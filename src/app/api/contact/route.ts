...
export async function POST(req: NextRequest) {
  try {
    ...
    const htmlBody = `
      <div style="font-family: monospace; max-width: 600px; margin: 0 auto; background: #060B18; color: #e0e0e0; padding: 32px; border-radius: 12px;">
        ...
        <table style="width: 100%; border-collapse: collapse;">
          ...
          ${zweck ? `<tr style="border-top: 1px solid #1a2035;"><td style="padding: 10px 0; color: #888; font-size: 12px;">Zweck</td><td style="padding: 10px 0; color: #e0e0e0; font-size: 14px;">${zweck}</td></tr>` : ''}
          ${betreff ? `<tr style="border-top: 1px solid #1a2035;"><td style="padding: 10px 0; color: #888; font-size: 12px;">Betreff</td><td style="padding: 10px 0; color: #e0e0e0; font-size: 14px;">${betreff}</td></tr>` : ''}
        </table>
    ...
    if (!apiKey) {
      // No API key yet — log and return success (setup pending)
      console.log('[CONTACT PENDING SETUP]', { name, email, zweck, betreff, ts: new Date().toISOString() });
      return NextResponse.json({ success: true });
    }

    // Lazy import to avoid module-level issues during build
    const { Resend } = await import('resend');
    const resend = new Resend(apiKey);

    const { error } = await resend.emails.send({
      from: 'sicherheit.ai Kontakt <onboarding@resend.dev>',
      to: TO_EMAIL,
      replyTo: email,
      subject: betreffLine,
      html: htmlBody,
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'E-Mail konnte nicht gesendet werden. Bitte direkt schreiben an: mahdi.mahmoud2008@gmail.com' }, { status: 500 });
    }
